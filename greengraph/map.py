...
class Map(object):
    def __init__(self, lat, long, satellite=True, zoom=10, 
        ...
        if satellite:
            params["maptype"]="satellite"    #Defines the map type as a satellite image.
            
        self.image = requests.get(base, 
                    params=params).content # Fetch our PNG image data
        content = BytesIO(self.image)      #Converts image to array.
        self.pixels= img.imread(content) # Parse our PNG image as a numpy array
        
    def green(self, threshold): #Checks if pixel is "mostly" green by some amount (threshold). Input is an array.
    ...
    def count_green(self, threshold = 1.1): #Sums all green pixels.
